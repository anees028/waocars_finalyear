		...
		if(!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Not enough stock available for this Part. Therefore, can\'t make a sale. Please select a different item.</div>';
					exit();
				}
				else {
					...
					if($customerStatement->rowCount() > 0){
						...
						echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button>Issue Part details added to DB and stocks updated.</div>';
						exit();
